...
public class CartScreenController {
	...
	public void initialize() {
		filteredCart = new FilteredList<>(this.cart.getItemsOrdered(), s -> true);
		...
		if (filter == null || filter.isEmpty()) {
			...
			if (filterByID) {
				try {
					...
					filteredCart.setPredicate(s -> false); // Reset filter on invalid input
				}
			} else {
				filteredCart.setPredicate(s -> s.getTitle().toLowerCase().contains(filter.toLowerCase()));
			}
		}
	}
